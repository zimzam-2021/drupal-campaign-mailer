<?php
namespace Drupal\controlpanel\API\Charts;
...
class PHPCharts
{
	...
	**/
	public function returnJS()
	{
		...
		if (in_array($this->type, array('bar', 'line', 'radar')) AND !empty($this->data) AND !empty($this->legend)) {
			$js = 'var '.$this->divName.'_data = {labels:[';
			...
			if (self::isMultidimensionalArray($this->data)) {
				...
				foreach ($this->data as $legend => $dataset) {
					$RGBcolor = self::HexadecimalColorToRGB(self::getNextColor());
					$js .= self::generateJSDataset($dataset, $RGBcolor, $legend );
				}
			}
			// Single dataset
			else
		...
		$js .= '
		phpchart_'.$this->divName.'();

		function phpchart_'.$this->divName.'(){
			var ctx = document.getElementById("'.$this->divName.'").getContext("2d");
            new Chart(ctx,{
                type: \''.$this->type.'\',
                data: '.$this->divName.'_data,
	...
	**/
	public function returnFullHTML()
	{
		...
		if (!$this->displayLegend)
			$html = '<canvas class="chart phpcharts chartcontainer" id="'.$this->divName.'" width="'.self::CANVAS_WIDTH_WITHOUT_LEGEND.'" height="'.self::CANVAS_HEIGHT_WITHOUT_LEGEND.'"></canvas>';
		else {
			$html = '<div class="chartContainer"><canvas class="chart phpcharts chartcontainer" id="'.$this->divName.'" width="'.self::CANVAS_WIDTH_WITH_LEGEND.'" height="'.self::CANVAS_HEIGHT_WITH_LEGEND.'"></canvas></div>';
	...
	* @return string The JS code for the chart
	**/
	private function generateJSDataset($data, $color, $legend = '')
	{
